...
const getOldChannels = async age => {
	...
	try {
		const channels = await wc.conversations.list({
			...
			count: 1000, //1000 is max count of channels - can get around this if necessary but it's annoying so not going to bother yet
		}).channels
		...
		for (channel of channels) {
			...
			if ((await lastMessageTs) - currentDate > age) {
...
const renameDeadChannel = async channelId => {
	try {
		...
		if (getChannelId(`zzz-${channelName}`) != null) {
